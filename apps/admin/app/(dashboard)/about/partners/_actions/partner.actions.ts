...
function validateRequiredLocales(
  translations:
    | CreatePartnerContentDto['translations']
    | UpdatePartnerContentDto['translations']
): ActionFailure | null {
  const ru = translations?.filter(
    (translation) => translation.locale === Locale.ru
  );
  const en = translations?.filter(
    (translation) => translation.locale === Locale.en
  );

  if (translations?.length === 2 && ru?.length === 1 && en?.length === 1) {
    return null;
  }
