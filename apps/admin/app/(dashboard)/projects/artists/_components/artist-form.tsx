...
export function ArtistForm({
  ...
  artistId?: string;
  initialValues: CreateArtistContentDto;
}) {
  const router = useRouter();
  const [formError, setFormError] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);
  const form = useForm<
    z.input<typeof createArtistContentDtoSchema>,
    undefined,
    CreateArtistContentDto
  >({
    resolver: zodResolver(createArtistContentDtoSchema),
    ...
    <form className="space-y-6" onSubmit={onSubmit}>
          ...
          <Tabs defaultValue="ru">
            ...
            {fields.map((translation, index) => (
              <TabsContent key={translation.id} value={translation.locale}>
                ...
                <FieldGroup>
                  <Field>
                    <FieldLabel htmlFor={`artist-title-${translation.locale}`}>
                      Название
                    </FieldLabel>
                    <Input
                      ...
                      id={`artist-title-${translation.locale}`}
                  ...
                  <Field>
                    ...
                    <Textarea
                      ...
                      id={`artist-text-${translation.locale}`}
      ...
      <div className="flex justify-end gap-3">
        <Button
          ...
          variant="outline"
        >
          <Link href="/projects/artists">Отменить</Link>
        </Button>
