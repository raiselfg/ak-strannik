...
import {
  ...
  updateConcertContent,
} from '../_actions/concert.actions';
export function ConcertForm({
  concertId,
  initialValues,
}: {
  concertId?: string;
  initialValues: CreateConcertContentDto;
}) {
  const router = useRouter();
  const [formError, setFormError] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);
  const form = useForm<
    z.input<typeof createConcertContentDtoSchema>,
    undefined,
    CreateConcertContentDto
  >({
    resolver: zodResolver(createConcertContentDtoSchema),
    ...
    <form className="space-y-6" onSubmit={onSubmit}>
          ...
          <Tabs defaultValue="ru">
            ...
            {fields.map((translation, index) => (
              <TabsContent key={translation.id} value={translation.locale}>
                ...
                <FieldGroup>
                  <Field>
                    <FieldLabel htmlFor={`concert-title-${translation.locale}`}>
                      Название
                    </FieldLabel>
                    <Input
                      ...
                      id={`concert-title-${translation.locale}`}
                  ...
                  <Field>
                    <FieldLabel
                      htmlFor={`concert-duration-${translation.locale}`}
                    >
                      Продолжительность
                    </FieldLabel>
                    <Input
                      aria-invalid={Boolean(
                        form.formState.errors.translations?.[index]?.duration
                      )}
                      id={`concert-duration-${translation.locale}`}
                      {...form.register(`translations.${index}.duration`)}
                    />
                    {form.formState.errors.translations?.[index]?.duration
                      ?.message ? (
                      <FieldError>
                  ...
                  <Field>
                    ...
                    <Textarea
                      ...
                      id={`concert-text-${translation.locale}`}
      ...
      <div className="flex justify-end gap-3">
        <Button asChild disabled={isSubmitting || uploading} variant="outline">
          <Link href="/projects/concerts">Отменить</Link>
        </Button>
