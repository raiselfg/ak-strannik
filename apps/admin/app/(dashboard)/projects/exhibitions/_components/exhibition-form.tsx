...
} from '../_actions/exhibition.actions';
export function ExhibitionForm({
  exhibitionId,
  initialValues,
}: {
  exhibitionId?: string;
  initialValues: CreateExhibitionContentDto;
}) {
  const router = useRouter();
  const [formError, setFormError] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);
  const form = useForm<
    z.input<typeof createExhibitionContentDtoSchema>,
    undefined,
    CreateExhibitionContentDto
  >({
    resolver: zodResolver(createExhibitionContentDtoSchema),
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    setFormError(null);
    const result = exhibitionId
      ? await updateExhibitionContent(exhibitionId, values)
      : await createExhibitionContent(values);
    if (!result.success) return setFormError(result.message);
    toast.success(result.message);
    router.push('/projects/exhibitions');
    router.refresh();
  });
  return (
    <form className="space-y-6" onSubmit={onSubmit}>
            ...
            {fields.map((translation, index) => (
              <TabsContent key={translation.id} value={translation.locale}>
                ...
                <FieldGroup>
                  <Field>
                    <FieldLabel
                      htmlFor={`exhibition-title-${translation.locale}`}
                    >
                      Название
                    </FieldLabel>
                    <Input
                      ...
                      id={`exhibition-title-${translation.locale}`}
      ...
      <div className="flex justify-end gap-3">
        <Button asChild disabled={isSubmitting || uploading} variant="outline">
          <Link href="/projects/exhibitions">Отменить</Link>
        </Button>
