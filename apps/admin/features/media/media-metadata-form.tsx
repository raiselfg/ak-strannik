...
export function MediaAssetMetadataForm({
  id,
  defaultValues,
}: {
  id: string;
  defaultValues: MediaAssetMetadataFormValues;
}) {
  const router = useRouter();
  const [error, setError] = useState<string | null>(null);
  const form = useForm<InputValues, unknown, MediaAssetMetadataFormValues>({
    resolver: zodResolver(MediaAssetMetadataFormSchema),
    defaultValues,
  });
  const submit = form.handleSubmit(async (values) => {
    ...
    if (!result.success) {
      for (const [name, messages] of Object.entries(result.fieldErrors ?? {}))
        form.setError(name as FieldPath<InputValues>, { message: messages[0] });
  ...
  return (
    <form className="space-y-5" onSubmit={submit}>
      ...
      <Button disabled={form.formState.isSubmitting} type="submit">
        {form.formState.isSubmitting ? 'Сохранение…' : 'Сохранить метаданные'}
      </Button>
    </form>
  );
}
...
function TranslationFields({
  ...
  locale: 'ru' | 'en';
  form: UseFormReturn<InputValues, unknown, MediaAssetMetadataFormValues>;
}) {
  ...
  return (
    <TabsContent value={locale}>
      <FieldGroup>
        <Field>
          <FieldLabel htmlFor={`${locale}-alt`}>Alt</FieldLabel>
          <Input
            id={`${locale}-alt`}
            aria-invalid={Boolean(errors?.alt)}
            {...form.register(`translations.${locale}.alt`)}
          />
          <FieldDescription>
            Краткое текстовое описание изображения для доступности и случаев,
            когда изображение не загрузилось.
          </FieldDescription>
          {errors?.alt?.message ? (
        ...
        <Field>
          ...
          />
          <FieldDescription>
            Дополнительный заголовок медиафайла.
          </FieldDescription>
        ...
        <Field>
          <FieldLabel htmlFor={`${locale}-caption`}>Caption</FieldLabel>
          <Textarea
            id={`${locale}-caption`}
            rows={5}
            aria-invalid={Boolean(errors?.caption)}
            {...form.register(`translations.${locale}.caption`)}
          />
          <FieldDescription>
            Подпись, которая может отображаться рядом с изображением.
          </FieldDescription>
          {errors?.caption?.message ? (
            <FieldError>{errors.caption.message}</FieldError>
          ) : null}
        </Field>
      </FieldGroup>
    </TabsContent>
