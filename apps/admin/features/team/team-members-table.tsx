    ...
    <Card>
      <CardContent className="px-0">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Фотография</TableHead>
          ...
          <TableBody>
            {members.map((member) => {
              const translation =
                member.translations.find((item) => item.locale === 'ru') ??
                ...
                <TableRow key={member.id}>
                  <TableCell>
                    <div
                      ...
                      title={member.image?.originalName}
                    >
                      {member.image ? (
                  ...
                  <TableCell>
                    <div className="flex justify-end gap-2">
                      <Button asChild size="sm" variant="outline">
                        <Link href={`/team/${member.id}`}>Редактировать</Link>
                      </Button>
                      <DeleteDialog
                        args={[member.id]}
